...
inventory[inventory['PartNo'] == 'VE55ULV-2SW-KIT']
...
Amazon_HC_result = get_dataframe_from_mazenda(12284)



Amazon_profile = pd.read_csv(r'P:\Dept - Sales\Ecommerce\Sales Reports\Greg Common Files\400 PROFILES.csv')
New_HC_Sku = pd.read_excel(r'P:\Dept - Sales\Ecommerce\Sales Reports\Greg Common Files\Master Skus\Fulfillment Master Sku List.xlsx')

New_HC_Sku.columns
New_HC_Sku = New_HC_Sku[New_HC_Sku['Fulfillment_CAT'] == 'Home Comfort']
New_HC_Sku = New_HC_Sku[['SKU', 'MFG','New', 'ASIN']]
New_HC_Sku = New_HC_Sku.rename(columns = {'MFG' : 'Mfr', 'SKU' : 'Model'})

New_HC_Sku[New_HC_Sku['Model'] == 'KSTAD5OB']
...
Amazon_HC_result = Amazon_HC_result[['Price', 'Description', 'Reviews', 'Ratings', 'Stock', 'LocationUrl', 'Error', 'Model', 'BuyBox']]
...
Part_1 = pd.merge(Amazon_HC_result, New_HC_Sku, how = 'right', on = ['Model'])
Part_3 = pd.merge(Part_1, Amazon_profile, how = 'left', on = ['Model'])
Part_4 = pd.merge(Part_3, inventory_detail_summed, how = 'left', on = ['Model'])

Part_4 = Part_4.rename(columns = {'New' : 'New Skus'})

Part_4 = Part_4[['Mfr', 'Model', 'ASIN', 'Category 1', 'Category 2', 'Ratings', 'Reviews', 'SumOfQAV', 'SumOfQOO', 'Dsc', 'BuyBox', 'Stock', 'Price', 'CUST$', 'MAP', 'New Skus', 'Description', 'Error', 'LocationUrl']]
...
map_file = pd.read_excel(r'P:\Dept - Sales\Ecommerce\Sales Reports\Greg Common Files\Amazon Suppression\Amazon - Focus sku mapping file.xlsx')
map_file.columns

map = map_file[['Mod+U1+A1:U90', 'Amazon ASIN']]
map = map.rename(columns = {'Mod+U1+A1:U90' : 'Model'})
map = map.rename(columns = {'Amazon ASIN' : 'ASIN'})


merge = pd.merge(Part_4, map, how = 'inner', on = ['Model', 'ASIN'])
Part_4 = Part_4.append(merge)

Part_4 = Part_4.drop_duplicates(subset = None, keep = False)

map_1 = map_file[['Mod+U1+A1:U90', 'Amazon ASIN', 'Summed Inventory']]
map_1 = map_1.rename(columns = {'Mod+U1+A1:U90' : 'Model'})
map_1 = map_1.rename(columns = {'Amazon ASIN' : 'ASIN'})


merge = pd.merge(map_1, merge, how = 'right', on = ['Model', 'ASIN'])
merge = merge.drop('SumOfQAV', axis = 1)
merge = merge.rename(columns = {'Summed Inventory' : 'SumOfQAV'})


merge = merge[['Mfr', 'Model', 'ASIN', 'Category 1', 'Category 2', 'Ratings', 'Reviews', 'SumOfQAV', 'SumOfQOO', 'Dsc', 'BuyBox', 'Stock', 'Price', 'CUST$', 'MAP', 'New Skus', 'Description', 'Error', 'LocationUrl']]



all = pd.concat([merge, Part_4], sort = False )
all.columns
len(all.columns)
all = all.rename(columns = {'BuyBox' : 'Buy Box'})

all.to_csv(r'C:\Users\Angela Qu\Desktop\test_0.csv', index=False)
...
def read_csv_to_mysql(filename):
    with codecs.open(filename=filename, mode='r', encoding='utf-8') as f:
        ...
        conn = db
        cur = conn.cursor()
        sql = 'insert into Amazon_HC values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
        count = 0
        for item in reader:
            args = tuple(item)
...
if __name__ == '__main__':
    read_csv_to_mysql(r'C:\Users\Angela Qu\Desktop\test_0.csv')
